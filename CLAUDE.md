...
- **Category delete is a cascade and is blocked by product usage.** `categoryService.softDeleteCategory` (the only delete path - single and bulk both call it) soft-deletes the category AND its whole subtree, and refuses with a 409 if any live (Active/Inactive) product references ANY category in that subtree via `mainCategory`/`subCategory` (`findProductUsage`). Any new code that deletes categories must go through it.
